...
class _JoinCreateScreenState extends State<JoinCreateScreen> {
  ...
  Future<void> _joinGroup() async {
    ...
    bool success = false;
    try {
      success = await context.read<NostalgiaProvider>().joinGroup(
  ...
  Future<void> _createGroup() async {
    ...
    bool success = false;
    try {
      success = await context.read<NostalgiaProvider>().createGroup(
            displayName: _nameController.text,
            avatarIcon: 'avatar_$_selectedAvatarIndex',
            avatarColor: (avatar['color'] as Color).toARGB32(),
  ...
  Widget build(BuildContext context) {
    ...
    final isBusy = provider.isCheckingAuth || _isJoining || _isCreating;
    final loadingText = provider.isCheckingAuth
        ? 'Checking your time machine...'
        : _isJoining
            ? 'Joining your crew...'
            : _isCreating
